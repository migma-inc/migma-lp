...
Deno.serve(async (req) => {
    ...
    try {
        ...
        if (!sellerId || !sellerEmail || !orderNumber) {
            return new Response(
                            ...
                            <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%">
                                <tr>
                                    <td style="padding: 30px; background: linear-gradient(135deg, #1a1a1a 0%, #000000 100%); border-radius: 8px; border: 1px solid #CE9F48;">
                                        ...
                                        <p style="margin: 0 0 20px 0; font-size: 16px; line-height: 1.6; color: #e0e0e0;">
                                            Great news! You have received a new payment for one of your sales.
                                        </p>
                                        <!-- Payment Details Box -->
                                        <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%">
                                            <tr>
                                        ...
                                        <p style="margin: 20px 0 0 0; font-size: 16px; line-height: 1.6; color: #e0e0e0;">
                                            You can view the complete order details in your seller dashboard.
                                        </p>
                                        <p style="margin: 20px 0 0 0; font-size: 16px; line-height: 1.6; color: #e0e0e0;">
    ...
    `;
        ...
        try {
            const emailResponse = await fetch(`${supabaseUrl}/functions/v1/send-email`, {
                ...
                body: JSON.stringify({
                    to: sellerEmail,
                    subject: `New Payment Received - Order ${safeOrderNumber}`,
                    html: emailHtml,
                }),
            });
            ...
            if (!emailResponse.ok) {
                ...
                console.error("[Seller Notification] Email error:", errorText);
            } else {
                const emailData = await emailResponse.json();
                console.log("[Seller Notification] Email sent successfully:", emailData);
            }
        } catch (emailErr) {
            console.error("[Seller Notification] Exception sending email:", emailErr);
        }

        return new Response(
            JSON.stringify({ success: true, message: "Seller notification sent" }),
            {
                status: 200,
                headers: {
                    "Content-Type": "application/json",
