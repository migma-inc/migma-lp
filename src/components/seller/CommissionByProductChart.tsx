...
import type { CommissionByProduct } from '@/lib/seller-analytics';
import { Coins, TrendingUp } from 'lucide-react';

interface CommissionByProductChartProps {
  data: CommissionByProduct[];
}

export function CommissionByProductChart({ data }: CommissionByProductChartProps) {
  // Sort products by commissions (highest to lowest)
  const sortedData = [...data].sort((a, b) => b.totalCommissions - a.totalCommissions).slice(0, 10);
  ...
  const totalCommissions = data.reduce((sum, p) => sum + p.totalCommissions, 0);
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-0">
          <CardTitle className="text-white text-base sm:text-lg flex items-center gap-2">
            <div className="p-1.5 sm:p-2 bg-gold-medium/20 rounded-lg">
              <Coins className="w-4 h-4 sm:w-5 sm:h-5 text-gold-light" />
            </div>
                ...
                <div className="flex items-start justify-between gap-2 sm:gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="ml-8 sm:ml-10 grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3 text-xs">
                      ...
                      <div>
                        <p className="text-gray-400 mb-1">Commissions</p>
                        <p className="text-gold-light font-bold">
                          ${product.totalCommissions.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                        </p>
                      </div>
                      <div>
                        <p className="text-gray-400 mb-1">Avg</p>
                        <p className="text-white font-medium">
                          ${product.avgCommission.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                        </p>
                      </div>
                      <div>
